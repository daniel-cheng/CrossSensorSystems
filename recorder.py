...
class VideoRecorder():
	...
	def __init__(self):
		...
		self.fourcc = "H264"       # capture images (with no decrease in speed over time; testing is required)
...
class AudioRecorder():
    ...
    def __init__(self):
        ...
        self.open = True
        self.rate = 16000 #Max rate for Logitech C525 webcam = 32000
...
#	 Merging audio and video signal
	...
	if abs(recorded_fps - 6) >= 0.01:    # If the fps rate was higher/lower than expected, re-encode it to the expected
		...
		print "Muxing"
		cmd = "ffmpeg -ac 2 -channel_layout stereo -i temp_audio.wav -i temp_video2.mp4 -pix_fmt yuv420p /var/www/html/videos/" + filename + ".mp4"
		subprocess.call(cmd, shell=True)
	
	else:
		
		print "Normal recording\nMuxing"
		cmd = "ffmpeg -ac 2 -channel_layout stereo -i temp_audio.wav -i temp_video.mp4 -pix_fmt yuv420p /var/www/html/videos/" + filename + ".mp4"
		subprocess.call(cmd, shell=True)
